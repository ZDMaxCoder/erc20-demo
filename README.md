...
- **充值检测** — 区块扫描 + 重组检测（parentHash 校验），可配置确认块数，Transfer 事件验证
- **提现处理** — 状态机生命周期管理，自动风控评估，幂等提交，自动重试与补偿
- **ERC-20 兼容** — 处理非标代币（USDT 无返回值、BNB approve-需先置零、bytes32 返回值），不支持 fee-on-transfer/rebasing 类型
- **Nonce 管理** — 基于 Redis 的分布式 Nonce 分配，Gap 检测与回收
- **Gas 策略** — 支持 EIP-1559，基于百分位的动态定价，estimateGas revert 检测
- **账户体系** — 乐观锁余额操作，long + exponent 金额模型（含溢出保护）
- **风控系统** — 可配置规则（金额限制、频率、黑名单），集成到提现创建流程
- **归集服务** — 自动将用户地址资金归集到热钱包（含 token 类型安全检查）
- **链上对账** — 每日链上 balanceOf 与平台账面对比，差异告警
- **监控告警** — 健康检查、细粒度告警去重、token 合约 admin 事件监控
