#!/bin/bash
#
# ERC-20 Adapter Foundation — 逐任务执行脚本
#
# 每个 task 使用独立的 Claude Code session，最小化上下文占用。
#
# 用法：
#   ./scripts/run-adapter-foundation.sh          # 从头开始，自动跳过已完成任务
#   ./scripts/run-adapter-foundation.sh 3.1      # 从 task 3.1 开始
#   ./scripts/run-adapter-foundation.sh 3.1 4.2  # 只执行 3.1 到 4.2
#

set -e

PROJECT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
CHANGE_NAME="erc20-adapter-foundation"
...
# ============================================================
# 解析 tasks.md 中所有未完成任务
# ============================================================
get_pending_tasks() {
  grep -n '^\- \[ \]' "$TASKS_FILE" | sed 's/^[0-9]*:- \[ \] //'
}
...
# ============================================================
# 获取与 task group 相关的 spec 文件
# ============================================================
get_spec_for_group() {
  local group_num="$1"
  case "$group_num" in
    1|2|3|5) echo "specs/erc20-adapter-models/spec.md" ;;
    4)       echo "specs/erc20-adapter-models/spec.md" ;;
    6)       echo "specs/erc20-return-value-decoding/spec.md" ;;
    7)       echo "" ;;  # 编译验证，无 spec
  esac
}
...
"
  ...
  cat <<EOF
你正在执行 ERC-20 适配层基础设施构建（change: ${CHANGE_NAME}）。
...
1. 阅读 openspec/changes/${CHANGE_NAME}/design.md 了解技术决策、包结构和接口设计。
2. 阅读 openspec/changes/${CHANGE_NAME}/tasks.md 了解任务上下文（已完成的任务表示已有的代码）。
${spec_instruction}4. 检查项目中已有代码以了解现有代码风格和依赖。
...
- 测试命名: 被测方法_场景_期望结果
- 完成后运行 mvn compile -pl erc20-platform-common,erc20-platform-blockchain 确认编译通过
- 如果本 task 包含"编写单元测试"，运行 mvn test -pl <对应模块> -Dtest=<TestClass> 验证通过

## 包路径约定

- 异常类: erc20-platform-blockchain/src/main/java/com/erc20/platform/blockchain/adapter/exception/
- 模型类: erc20-platform-blockchain/src/main/java/com/erc20/platform/blockchain/adapter/model/
- RPC 类: erc20-platform-blockchain/src/main/java/com/erc20/platform/blockchain/adapter/rpc/
- 枚举 (TokenCapability/RiskLevel): erc20-platform-common/src/main/java/com/erc20/platform/common/enums/
- 测试: 对应模块的 src/test/java/ 下同包结构

## 质量要求

- 类为 final 或提供明确扩展点
- 值对象字段使用 private final
- 提供有意义的 toString() 实现
- 异常类提供 serialVersionUID
- 所有地址字段统一小写
...
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  ERC-20 Adapter Foundation — 逐任务执行                   ║"
echo "╠══════════════════════════════════════════════════════════╣"
...
while IFS= read -r line; do
  ...
  if echo "$line" | grep -qE '^\- \[[ x]\] [0-9]+\.[0-9]+'; then
    ...
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "[START] Task ${task_id}: ${task_desc:0:60}..."
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    if claude -p "$PROMPT" \
      ...
      if is_task_done "$task_id"; then
        ...
        if [[ "$REPLY" =~ ^[Rr]$ ]]; then
          ...
          echo "  已停止。重启: ./scripts/run-adapter-foundation.sh $task_id"
          break
        fi
      fi
    else
...
cd "$PROJECT_DIR"
if mvn compile -pl erc20-platform-common,erc20-platform-blockchain -q 2>&1 | tail -3; then
  echo "  [PASS] 编译通过 ✓"
else
  echo "  [WARN] 编译有问题"
fi

if mvn test -pl erc20-platform-common,erc20-platform-blockchain -q 2>&1 | tail -5; then
  echo "  [PASS] 测试通过 ✓"
else
  echo "  [WARN] 测试有失败"
fi
